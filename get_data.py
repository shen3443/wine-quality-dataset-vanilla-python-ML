...
class GetData:
    ...
    *input values have been adjusted such that:
    ...
    def __init__(self, path):
        ...
        with open(path) as csvfile:  # open csv file
            ...
            for row in datareader:
                ...
                self.ytrue.extend(rowlist[-1])  # expected outputs to ytrue
        ...
        for y in self.ytrue:
            # wines with scores lower than 5 are "bad" (0)
            if y < 5:
                ytrue_temp.append(0)
            # wines with scores higher than 6 are "good" (2)
            elif y > 6:
                ytrue_temp.append(2)
            # wines with scores in between are "okay" (1)
            else:
                ytrue_temp.append(1)
    ...
    def feature_scale(self, technique):
        data_invert = [[row[i] for row in self.data]
                       for i in range(len(self.data[0]))]

        if technique == "standardize":
            feature_means = [sum(col)/len(col) for col in data_invert]
            feature_standard_deviations = [math.sqrt(
                    sum([(x - mu)**2 for x in col])/len(col)
                    )
                for mu, col in zip(feature_means, data_invert)]
            data_temp = [[(x - mu)/sigma for x, mu, sigma in zip(
                        row, feature_means,
            ...
            feature_mins, feature_maxs = zip(
                *[(min(col), max(col)) for col in data_invert]
                )
            data_temp = [
                [(x - x_min) / (x_max - x_min) for x, x_min, x_max in
                    zip(row, feature_mins, feature_maxs)]
                for row in self.data]
